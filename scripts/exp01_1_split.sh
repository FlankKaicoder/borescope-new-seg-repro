...
mkdir -p "$OUT"
printf '%q ' "$PYTHON_BIN" "$PROJECT_ROOT/tools/dataset/build_dataset_v1.py" split --data-root "$DATA_ROOT" --conversion "$CONVERSION" --output "$OUT/artifacts" --seed 42 --restarts 6 --iterations 300 > "$OUT/command.txt"; printf '\n' >> "$OUT/command.txt"
"$PYTHON_BIN" "$PROJECT_ROOT/tools/dataset/build_dataset_v1.py" split --data-root "$DATA_ROOT" --conversion "$CONVERSION" --output "$OUT/artifacts" --seed 42 --restarts 6 --iterations 300 2>&1 | tee "$OUT/run.log"
