...
OUT="$ROOT/results/exp12_simsiam_basic/smoke_${RUN_ID}"

cd "$ROOT"
export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"

if [[ -e "$OUT" ]]; then
  echo "Refusing to overwrite existing Exp12.1-S output: $OUT" >&2
  exit 2
fi

exec .venv/bin/python -m tools.ssl.exp12_simsiam_smoke \
  --project-root "$ROOT" \
