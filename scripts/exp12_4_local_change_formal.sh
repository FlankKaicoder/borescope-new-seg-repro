...
if [[ $# -ne 1 ]]; then
  echo "usage: $0 /absolute/path/to/passed-smoke-summary.json" >&2
  exit 2
fi
...
OUT="$ROOT/results/exp12_local_change/formal_${RUN_ID}"

cd "$ROOT"
export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"

if [[ ! -f "$SMOKE_SUMMARY" ]]; then
  echo "Smoke summary does not exist: $SMOKE_SUMMARY" >&2
  exit 2
fi
if [[ -e "$OUT" ]]; then
  echo "Refusing to overwrite existing Exp12.4 output: $OUT" >&2
  exit 2
fi
