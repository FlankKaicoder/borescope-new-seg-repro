...
model.add_callback("on_pretrain_routine_end",save_initial);model.add_callback("on_fit_epoch_end",stop_after_six)
...
for logical in range(1,7):
    path=save_dir/"weights"/f"epoch{logical-1}.pt";entries.append({"checkpoint_epoch":logical,"training_file_epoch":logical-1,"path":str(path)})
for e in entries:
