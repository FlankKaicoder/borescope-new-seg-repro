...
def main() -> int:
    ...
    for i,c in enumerate(metrics.box.ap_class_index):
        ...
        rows.append({"class_id":int(c),"class_name":metrics.names[int(c)],"gt_images":int(metrics.box.nt_per_image[int(c)]),"gt_instances":int(metrics.box.nt_per_class[int(c)]),"box_precision":bp,"box_recall":br,"box_ap50":b50,"box_ap50_95":b95,"mask_precision":mp,"mask_recall":mr,"mask_ap50":m50,"mask_ap50_95":m95})
    with (a.output/"per_class_metrics.csv").open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0])); w.writeheader(); w.writerows(rows)
    print(json.dumps({"status":"PASS","split":a.split,"overall":overall,"classes":len(rows)},indent=2))
    return 0
